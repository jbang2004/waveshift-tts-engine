...
def decode_one_audio_mossformer2_se_48k(model, device, inputs, args):
    ...
    if input_len > args.sampling_rate * args.one_time_decode_length:  # 20 seconds
        ...
        if online_decoding:
            ...
            noise_outputs = torch.from_numpy(np.zeros(t))  # Initialize noise output tensor
            ...
            while current_idx + window <= t:
                ...
                # 重建噪声：使用反向掩码
                noise_mask = 1 - pred_mask
                noise_spec = spectrum.cpu() * noise_mask.detach().cpu()
                ...
                if current_idx == 0:
                    ...
                    # 写入对应的噪声音频
                    noise_seg_last = noise_segment[-window:]
                    noise_outputs[current_idx + give_up_length:current_idx + window - give_up_length] = noise_seg_last[give_up_length:-give_up_length]
                current_idx += stride  # Move to the next segment
        ...
        # 同步重建噪声音频
        noise_mask = 1 - pred_mask
        noise_spec = spectrum * noise_mask.detach().cpu()
        noise_spec_complex = noise_spec[:, :, 0] + 1j * noise_spec[:, :, 1]
        noise_outputs = istft(noise_spec_complex, args, len(audio))

    # 截断到原始输入长度，避免后续广播错误
    outputs = outputs[:input_len]
    noise_outputs = noise_outputs[:input_len]
    # 返回增强后波形和噪声音频数组
    enhanced = outputs.numpy() / MAX_WAV_VALUE
    noise = noise_outputs.numpy() / MAX_WAV_VALUE
