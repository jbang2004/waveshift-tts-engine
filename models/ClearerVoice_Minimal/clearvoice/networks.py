...
class SpeechModel:
    ...
    def decode(self):
        ...
        for i in range(len(self.data['audio'])):
            decoded = decode_one_audio(self.model, self.device, self.data['audio'][i], self.args)
            # Check if decoded result is a tuple (enhanced, noise)
            if isinstance(decoded, tuple) and len(decoded) == 2:
                output_audio, output_noise = decoded
                has_noise = True
            else:
                output_audio, output_noise = decoded, None
            ...
            if isinstance(output_audio, list):
                ...
                for spk in range(self.args.num_spks):
                    ...
                    if output_noise is not None:
                        output_noise[spk] = output_noise[spk][:self.data['audio_len']]
            else:
                # Single output, truncate to input audio length
                output_audio = output_audio[:self.data['audio_len']]
                if output_noise is not None:
                    output_noise = output_noise[:self.data['audio_len']]
            output_audios.append(output_audio)
            if has_noise:
                noise_audios.append(output_noise)
            ...
            for spk in range(self.args.num_spks):
                ...
                if has_noise:
                    ...
                    for i in range(len(noise_audios)):
                        noise_audio_buf.append(noise_audios[i][spk])
                    noise_audios_np.append(np.array(noise_audio_buf))
            if has_noise:
                return output_audios_np, noise_audios_np
            else:
                return output_audios_np
        else:
            output_audios_np = np.array(output_audios)
            if has_noise:
                noise_audios_np = np.array(noise_audios)
                return output_audios_np, noise_audios_np
            else:
                return output_audios_np
        ...
        if self.args.task == 'target_speaker_extraction':
            ...
            with torch.no_grad():
                for idx in tqdm(range(num_samples)):  # Loop over all audio samples
                    ...
                    res = self.decode()
                    # Check if result is tuple (enhanced, noise)
                    if isinstance(res, tuple) and len(res) == 2:
                        enhanced, noise = res
                    else:
                        enhanced, noise = res, None
                    output_audios = enhanced
                        ...
                        if len(scalars) > 1:
                            for i in range(len(scalars)):
                                ...
                                if noise is not None:
                                    noise[:, i] = noise[:, i] * scalars[i]
                        else:
                            output_audios = output_audios * scalars[0]
                            if noise is not None:
                                noise = noise * scalars[0]
                    ...
                    if online_write:
                        ...
                        if isinstance(output_audios, list):
                            ...
                            # Save noise audio if available
                            if noise is not None:
                                noise_file = output_file.replace('.' + self.data['ext'], '_noise.' + self.data['ext'])
                                self.write_audio(noise_file, key=None, spk=None, audio=noise)
                    else:
                        # If not writing to disk, store the output in the result dictionary
                        # 返回字典包含增强和噪声
                        if noise is not None:
                            self.result[wav_id] = {'enhanced': output_audios, 'noise': noise}
                        else:
