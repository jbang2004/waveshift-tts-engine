...
class network_wrapper(nn.Module):
    ...
    def load_args_se(self):
        ...
        # 设置绝对路径的checkpoints目录
        checkpoints_dir = os.path.join(self.project_root, "checkpoints", self.model_name)
        ...
        # 关键修改：强制设置checkpoint_dir为绝对路径，确保YAML中的设置不会覆盖
        self.args.checkpoint_dir = checkpoints_dir
    ...
    def load_args_ss(self):
        ...
        parser.add_argument('--config', default=str(self.config_path), help="Path to configuration file", action=yamlargparse.ActionConfigFile)
        parser.add_argument('--mode', type=str, default='inference', help='Modes: train or inference')
        parser.add_argument('--checkpoint-dir', dest='checkpoint_dir', type=str, default=checkpoints_dir, help='Checkpoint directory')
